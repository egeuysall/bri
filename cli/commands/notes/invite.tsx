...
export const description = 'Invite a user to a note';
...
export default function NotesInvite({ args: parsedArgs, options: parsedOptions }: Props) {
  return (
    <RunCommand
      label="Inviting user"
      json={parsedOptions.json}
      updateCheck={parsedOptions.updateCheck}
      action={(command) => runInvite('note', parsedArgs[0], parsedOptions as InviteOptions, command)}
    />
  );
}
